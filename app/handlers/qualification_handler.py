...
async def handle_unqualification(db: Session, msg_text: str, sender: str, reply_url: str, source: str, status: str):
    ...
    if not company_name:
        ...
        return send_message(reply_url, sender, "Which company are you referring to?", source)
    ...
    if lead.assigned_to:
        ...
        if assignee and assignee.username != sender_name:
            ...
            send_whatsapp_message(None, assignee.usernumber, notification)

    return send_message(reply_url, sender, f"✅ Understood. Lead for '{company_name}' has been marked as '{status_text}'.", source)
